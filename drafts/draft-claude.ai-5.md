...
**Depends on:** [RFC-0](./draft-claude.ai-0.md) (Terminology), [RFC-1](./draft-claude.ai-1.md) (DIDs, Envelopes), [RFC-2](./draft-claude.ai-2.md) (Capability Tokens), [RFC-3](./draft-claude.ai-3.md) (Interest Documents)
...
- A `sian:` URI may resolve to _different_ underlying mechanisms depending on context and DID method ([RFC-1](./draft-claude.ai-1.md) §1.1) — sometimes `did:web` (HTTPS resolution), sometimes `did:key` (no resolution at all, self-certifying), sometimes a gossip-propagated envelope ([RFC-2](./draft-claude.ai-2.md)) with no fixed network location. No single existing scheme covers all three uniformly.
...
Wraps a DID ([RFC-1](./draft-claude.ai-1.md) §1) for use in contexts (QR codes, deep links, NFC taps) expecting a URI rather than a bare DID string.
...
Addresses a specific signed envelope ([RFC-1](./draft-claude.ai-1.md) §3) by content hash, not location — consistent with §2's non-locator design, since an envelope may be cached at many grid nodes simultaneously ([RFC-2](./draft-claude.ai-2.md) §2).
...
Addresses an `InterestDocument` ([RFC-3](./draft-claude.ai-3.md) §1), typically referenced from within an envelope's `payload` field rather than fetched independently.
...
Addresses a short-lived, single-use capability token ([RFC-2](./draft-claude.ai-2.md) §1) used to gate meetup-class disclosure ([RFC-4](./draft-claude.ai-4.md) §3.2). Per [RFC-4](./draft-claude.ai-4.md) §3, tokens of this type are inherently single-resolution — dereferencing one twice MUST fail on the second attempt.
...
Addresses a specific grid node ([RFC-2](./draft-claude.ai-2.md) §1) for federation/peering configuration, node-operator policy lookup, or debugging.
...
Because `sian:` deliberately does not fix a resolution mechanism (§2), a resolver MUST branch on `sian-type` and, for `did`, on the DID method ([RFC-1](./draft-claude.ai-1.md) §1.1):

| Type                | Resolution                                                                                 |
| ------------------- | ------------------------------------------------------------------------------------------ |
| `sian:did:key:...`  | Self-certifying; no network resolution needed.                                             |
| `sian:did:web:...`  | HTTPS fetch per `did:web` spec.                                                            |
| `sian:did:peer:...` | Resolved from local pairwise log only; not globally resolvable ([RFC-1](./draft-claude.ai-1.md) §1.1). |
| `sian:env:...`      | Look up by content hash in local cache, then gossip network ([RFC-2](./draft-claude.ai-2.md) §2).      |
...
A resolver that cannot resolve a given `sian-type`/method combination MUST fail closed (consistent with [RFC-1](./draft-claude.ai-1.md) §2.2 and §5's fail-closed principle for unrecognized algorithms) rather than guess at a fallback mechanism.

## 5. Security Considerations

- **Correlation risk via `sian:cap:` reuse.** A capability token URI (§3.4) that is dereferenced more than once, or shared outside its intended single recipient, defeats the ephemerality property it exists for ([RFC-2](./draft-claude.ai-2.md) §1, [RFC-4](./draft-claude.ai-4.md) §3.2). Implementations MUST enforce single-use at the issuing node, not merely at the client.
- **`sian:did:web:` inherits HTTPS/DNS trust assumptions.** As with `did:web` generally ([RFC-1](./draft-claude.ai-1.md) §1.1), a `sian:did:web:` URI is only as trustworthy as the domain's DNS and TLS chain — this scheme adds no additional guarantee beyond what `did:web` itself provides.
...
Per RFC 7595 §7, provisional registration requires: scheme name (`sian`), status (provisional), applications/protocols using it (this document series), contact/change controller (community BCP process, [RFC-0](./draft-claude.ai-0.md) §1), and a reference to this specification. No permanent registration is sought until the protocol sees broader deployment (RFC 7595 §3.1's threshold for permanent status).

## 7. Open Issues

- OI-1: Whether `sian:env:` and `sian:doc:` should be merged into a single content-addressed type, since both are hash-addressed and the distinction may not earn its keep — flagged for review alongside [RFC-3](./draft-claude.ai-3.md)'s ontology governance gap.
