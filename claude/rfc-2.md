...
This document specifies how user devices discover grid nodes and each other, how information propagates between grid nodes over distance, and how staleness arising from disconnected/intermittent contact is handled. It explicitly does not attempt to guarantee freshness — see [RFC-0](./rfc-0.md) P3.
...
- On approach, device and node exchange signed Envelopes ([RFC-1](./rfc-1.md) §3) at minimum containing: DID, a capability advertisement (what the node offers: events, ambient activity broadcasts, etc.), and TTL-bound interest/disposition data the device is willing to disclose at this trust tier.
- **Capability tokens for meetup-class exchanges:** where a handshake could lead to resolving precise location/time ([RFC-4](./rfc-4.md) §2's "IRL-meetup-class" actions), the initial exchange MUST use a short-lived, single-use capability token rather than a durable pointer, to avoid creating a stable correlation handle across visits (see [RFC-1](./rfc-1.md) §1.2, [RFC-4](./rfc-4.md) §3).
...
- Nodes choose their federation peers voluntarily ([RFC-0](./rfc-0.md) P1); there is no mandated topology.
- Propagated envelopes retain their original [RFC-1](./rfc-1.md) signatures; intermediate nodes are transport-only and are NOT trusted for integrity, only availability ([RFC-1](./rfc-1.md) §3.2).
- Each node applies its own local policy for what it accepts, retransmits, and how far (see [RFC-4](./rfc-4.md) §1 for rate-limit mechanisms situated at this layer).
...
Per [RFC-0](./rfc-0.md) P3, this is an accepted property:

- Envelopes carry `revision` and `ttl_seconds` ([RFC-1](./rfc-1.md) §3.1); devices merge on receipt using last-writer-wins by revision.
