...
[RFC-4](./rfc-4.md) §3.2 requires graduated disclosure for meetup-class actions but leaves the concrete rungs unspecified. This document defines a specific eight-rung reveal ladder (R0–R7) between two avatars, the identity-tier and cost requirements gating advancement, and the rules governing skipping, reversal, and mutual consent.

## 1. The Ladder

| Rung | Disclosure                                                                   | Minimum identity tier ([RFC-1](./rfc-1.md) §1.2) | Requires mutual consent?                 |
| ---- | ---------------------------------------------------------------------------- | ------------------------------------------------ | ---------------------------------------- |
| R0   | Anonymous opportunity (ambient broadcast visibility, [RFC-2](./rfc-2.md) §1) | None                                             | No — passive                             |
| R1   | Pseudonymous avatar (DID exchanged)                                          | `did:key`                                        | No — one-directional, automatic on match |
| R2a  | Shared interests (ordinary interest-doc match, [RFC-3](./rfc-3.md))          | `did:key`                                        | No — automatic on match                  |
...
Per [RFC-4](./rfc-4.md) §1's action-class table, an avatar attempting to advance past R3 while still on a `did:key` identity SHOULD trigger the step-up flow defined in [RFC-4](./rfc-4.md) §3.2 (prompt to upgrade to `did:peer`/`did:web`, or accept a posted bond as an alternative per [RFC-4](./rfc-4.md) §2.3) before the receiving avatar's client permits the grant.

### 2.5 Continuity across DID migration

If an avatar migrates DIDs mid-relationship ([RFC-1](./rfc-1.md) §6.2), the granted-rung state for that pairwise relationship SHOULD carry forward under the same Migration Statement mechanism and cost constraints as reputation ([RFC-4](./rfc-4.md) §3.4) — a migration MUST NOT be usable to silently re-request a rung that was previously revoked by the counterpart, since that would let migration function as a consent-reset exploit.

## 3. Security and Safety Considerations

- **R2b as a safety-sensitive rung.** Stated dating/relational intent is exactly the kind of disclosure the original proposal's safety warning ([RFC-0](./rfc-0.md) §5) was concerned with — it signals availability in a way that can attract predatory behavior. Implementations SHOULD apply the same or stricter scrutiny to R2b as to meetup-class actions generally ([RFC-4](./rfc-4.md) §1), even though R2b alone does not yet disclose location or contact info.
...
This document supersedes the placeholder language in [RFC-4 ](./rfc-4.md)§3.2 ("coarse information only... precise location/time disclosed only after mutual, explicit opt-in") with the concrete R0–R7 ladder above. [RFC-4](./rfc-4.md) §3.2 SHOULD be amended to reference this document directly rather than describing graduated disclosure in the abstract.

## 5. Open Issues

- OI-1: Whether R2b should have its own distinct cost-of-action requirement ([RFC-4](./rfc-4.md) §2) beyond identity-tier flagging, given its safety sensitivity, is unresolved.
